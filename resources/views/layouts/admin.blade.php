...
<head>
    ...
    <title>@yield('title', 'Admin - Perpustakaan SMAN 7 Pekanbaru')</title>
        ...
        <aside class="w-72 bg-gradient-to-b from-primary-dark to-green-900 fixed left-0 top-0 h-screen text-white overflow-y-auto">
            <div class="p-6">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3">
                    @if(file_exists(public_path('logo-sekolah.jpg')))
                        <img src="{{ asset('logo-sekolah.jpg') }}" alt="Logo" class="w-12 h-12 rounded-xl object-cover">
                    @else
                    ...
                    <div>
                        ...
                        <span class="text-white/70 text-xs block">SMAN 7 Pekanbaru</span>
                    </div>
                </a>
            </div>
            ...
            <nav class="px-4 space-y-1">
                ...
                <a href="{{ route('admin.dashboard') }}" 
                   class="{{ request()->routeIs('admin.dashboard*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                ...
                <a href="{{ route('admin.books.index') }}" 
                   class="{{ request()->routeIs('admin.books*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('admin.borrowings.index') }}" 
                   class="{{ request()->routeIs('admin.borrowings*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-4 mt-4 border-t border-white/10">
                    <p class="px-4 text-white/40 text-xs uppercase tracking-wider mb-2">Master Data</p>
                    
                    <a href="{{ route('admin.students.index') }}" 
                       class="{{ request()->routeIs('admin.students*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('admin.grades.index') }}" 
                       class="{{ request()->routeIs('admin.grades*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('admin.shelves.index') }}" 
                       class="{{ request()->routeIs('admin.shelves*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ route('admin.categories.index') }}" 
                       class="{{ request()->routeIs('admin.categories*') ? 'bg-white/20 text-white' : 'text-white/70 hover:bg-white/10' }} flex items-center gap-3 px-4 py-3 rounded-xl transition font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Other -->
                <div class="pt-4 mt-4 border-t border-white/10">
                    <p class="px-4 text-white/40 text-xs uppercase tracking-wider mb-2">Lainnya</p>
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4">
                <div class="bg-white/5 rounded-xl p-3 text-center">
                    <p class="text-white/40 text-xs">© {{ date('Y') }} SMAN 7 Pekanbaru</p>
                </div>
            </div>
        </aside>
        ...
        <main class="ml-72 flex-1 min-h-screen flex flex-col">
            ...
            <footer class="bg-white border-t border-gray-100 px-8 py-4 mt-auto">
                <div class="flex justify-between items-center text-gray-500 text-sm">
                    <p>© {{ date('Y') }} Perpustakaan SMAN 7 Pekanbaru</p>
