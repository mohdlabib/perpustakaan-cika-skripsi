...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
    ...
    <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm {{ \App\Models\Borrowing::overdue()->count() > 0 ? 'border-red-200 bg-red-50' : '' }}">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 {{ \App\Models\Borrowing::overdue()->count() > 0 ? 'bg-red-100' : 'bg-green-100' }} rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6 {{ \App\Models\Borrowing::overdue()->count() > 0 ? 'text-red-600' : 'text-green-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('admin.borrowings.import') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-3 p-3 bg-blue-50 rounded-xl text-xs text-blue-800 space-y-1">
                        ...
                        <p>• Kolom: NIS, Kode Eksemplar, Judul Buku, Tanggal Pinjam, Batas Kembali, Status</p>
                        <p>• Peminjaman yang sudah ada (NIS + Buku + Tanggal sama) akan di-skip</p>
                        <p>• Atau download <a href="{{ route('admin.borrowings.template') }}" class="underline font-medium">template kosong</a></p>
                    </div>
                </div>
                <div class="flex gap-3 pt-2">
