...
@section('page-title', 'Edit Eksemplar - ' . $book->title)
...
<div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
    ...
    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-gray-800 font-medium">Edit Eksemplar</span>
</div>

<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-800">Edit Eksemplar</h2>
            <p class="text-gray-500 text-sm">Buku: <strong>{{ $book->title }}</strong> | Kode: <strong>{{ $copy->copy_code ?? '#'.$copy->id }}</strong></p>
        </div>

        <form action="{{ route('admin.books.copies.update', [$book, $copy]) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="copy_code" value="{{ old('copy_code', $copy->copy_code) }}"
                ...
                <div>
                    ...
                    <input type="text" name="inventory_code" value="{{ old('inventory_code', $copy->inventory_code) }}"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark focus:border-transparent">
                    @error('inventory_code') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="grid grid-cols-2 gap-5" x-data="{ shelfId: '{{ old('shelf_id', $copy->shelf_id ?? '') }}', columns: [] }" x-init="if(shelfId) fetch('/admin/shelves/' + shelfId + '/columns').then(r => r.json()).then(d => columns = d)">
                <div>
                    ...
                    <select name="shelf_id" x-model="shelfId" @change="if(shelfId) fetch('/admin/shelves/' + shelfId + '/columns').then(r => r.json()).then(d => columns = d); else columns = []"
                        ...
                        @foreach($shelves as $shelf)
                            <option value="{{ $shelf->id }}" {{ old('shelf_id', $copy->shelf_id) == $shelf->id ? 'selected' : '' }}>{{ $shelf->code }} - {{ $shelf->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="shelf_column_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark cursor-pointer">
                        ...
                        <template x-for="col in columns" :key="col.id">
                            <option :value="col.id" x-text="'Kolom ' + col.name" :selected="col.id == {{ old('shelf_column_id', $copy->shelf_column_id ?? 0) }}"></option>
                        </template>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="condition" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark cursor-pointer">
                        <option value="baik" {{ old('condition', $copy->condition) == 'baik' ? 'selected' : '' }}>Baik</option>
                        <option value="rusak" {{ old('condition', $copy->condition) == 'rusak' ? 'selected' : '' }}>Rusak</option>
                        <option value="hilang" {{ old('condition', $copy->condition) == 'hilang' ? 'selected' : '' }}>Hilang</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Diterima</label>
                    <input type="date" name="received_date" value="{{ old('received_date', $copy->received_date?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark focus:border-transparent">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $copy->price) }}" min="0" step="100"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark focus:border-transparent">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-dark focus:border-transparent">{{ old('notes', $copy->notes) }}</textarea>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-primary-dark text-white rounded-xl font-medium hover:bg-opacity-90 transition cursor-pointer">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
